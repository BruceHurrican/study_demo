...
public class BlueToothChatService {
    ...
    private synchronized void setState(int state) {
        if (D) {
            Logs.d(TAG, "setState()方法执行 " + mState + " --> " + state);
        }
        mState = state;
     ...
     *
     * @param socket     The BluetoothSockey on which the connection was made
     * @param device     The BluetoothDevice that has been connected
     * @param socketType
     */
    public synchronized void connected(BluetoothSocket socket, BluetoothDevice device, final String socketType) {
        if (D) {
            Logs.d(TAG, "connected,Socket Type :" + socketType);
        }
    ...
    private class AcceptThread extends Thread {
        ...
        public void run() {
            if (D) {
                Logs.d(TAG, "Socket Type :" + mSocketType + " BEGIN mAcceptThread" + this);
            }
                    ...
                    synchronized (BlueToothChatService.this) {
                        switch (mState) {
                            ...
                            case STATE_CONNECTED:
                                // Either not ready or already connected. Terminate new sockey.
                                try {
                                    socket.close();
                                } catch (IOException e) {
                                    Log.e(TAG, "Could not close unwanted sockey", e);
                                }
                                break;
                        }
                    }
            ...
            if (D) {
                Logs.d(TAG, "END mAcceptThread, sockey Type:" + mSocketType);
            }
        }

        public void cancel() {
            if (D) {
                Logs.d(TAG, "Socket Type " + mSocketType + " cancel " + this);
            }
            try {
     ...
     * with a device. It runs straight through; the connection either success or fails.
     */
    private class ConnectThread extends Thread {
        ...
        public void run() {
            Logs.d(TAG, "BEGIN mConnectThread SocketType: " + mSocketType);
            setName("ConnectThread " + mSocketType);

            // Always cancel discovery because it will slow down a connection
            mAdapter.cancelDiscovery();

            // Make a cnnection to the BluetoothSocket
            try {
    ...
    private class ConnectedThread extends Thread {
        ...
        public ConnectedThread(BluetoothSocket socket, String socketType) {
            Logs.d(TAG, "create ConnectedThread :" + socketType);
            mmSocket = socket;
        ...
        public void run() {
            Logs.d(TAG, "BEGIN mConnectedThread");
